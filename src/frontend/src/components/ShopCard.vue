<template>
  <div class="relative z-0 flex flex-col self-start overflow-hidden leading-5 bg-white rounded-lg shadow-lg">
          ...
          <dl class="text-sm">
            <div class="owl:mt-1 flex flex-col">
              <dt class="owl:ml-1 flex items-center text-gray-400">
                ...
                <span>Real-time Population Density:</span>
              </dt>
              <dd class="font-medium">
                <button
                  ...
                  ><span>{{ (Math.abs(realtimeData.doorAIn + realtimeData.doorBIn - realtimeData.doorAOut - realtimeData.doorBOut )/60).toFixed(2) }}</span>
    ...
    <div v-if="open" class="bg-gray-50 sm:px-6 owl:mt-8 flex flex-col px-4 py-6 border-t-2 border-gray-100">
      ...
      <p class="w-full"><small class="text-lg">Updated realtime by
        <span class="bg-gray-300-25 hover:bg-gray-100 focus:bg-gray-100 px-2 py-1 rounded">
          <code class="font-mono">1 sec</code>
        </span>
        ago
      </small></p>
      ...
      <div class="w-full">
        <dl class="grid grid-cols-1 row-gap-1 col-gap-12">
          ...
          <div class="owl:ml-2 flex justify-between text-sm">
            <dt class="owl:ml-1 flex items-center text-gray-500">Head Count</dt>
            <dd class="owl:ml-1 flex items-center leading-none text-right">
              <span class="w-24 px-2 py-1 text-right bg-white border border-gray-200 rounded">{{ Math.abs(realtimeData.doorAIn + realtimeData.doorBIn - realtimeData.doorAOut - realtimeData.doorBOut)  }}</span>
            </dd>
          </div>

          <div class="owl:ml-2 flex justify-between text-sm">
            <dt class="owl:ml-1 flex items-center text-gray-500">Population Density</dt>
            <dd class="owl:ml-1 flex items-center leading-none text-right">
              <span class="w-24 px-2 py-1 text-right bg-white border border-gray-200 rounded">{{(Math.abs(realtimeData.doorAIn + realtimeData.doorBIn - realtimeData.doorAOut - realtimeData.doorBOut )/60).toFixed(2) }} 😀 / m²</span>
            </dd>
          </div>

          <div class="owl:ml-2 flex justify-between text-sm">
            <dt class="owl:ml-1 flex items-center text-gray-500">Door A Enter Count</dt>
            <dd class="owl:ml-1 flex items-center leading-none text-right">
              <span class="w-24 px-2 py-1 text-right bg-white border border-gray-200 rounded">{{ realtimeData.doorAIn }}</span>
            </dd>
          </div>

          <div class="owl:ml-2 flex justify-between text-sm">
            <dt class="owl:ml-1 flex items-center text-gray-500">Door A Exit Count</dt>
            <dd class="owl:ml-1 flex items-center leading-none text-right">
              <span class="w-24 px-2 py-1 text-right bg-white border border-gray-200 rounded">{{ realtimeData.doorAOut }}</span>
            </dd>
          </div>

          <div class="owl:ml-2 flex justify-between text-sm">
            <dt class="owl:ml-1 flex items-center text-gray-500">Door B Enter Count</dt>
            <dd class="owl:ml-1 flex items-center leading-none text-right">
              <span class="w-24 px-2 py-1 text-right bg-white border border-gray-200 rounded">{{ realtimeData.doorBIn }}</span>
            </dd>
          </div>

          <div class="owl:ml-2 flex justify-between text-sm">
            <dt class="owl:ml-1 flex items-center text-gray-500">Door B Exit Count</dt>
            <dd class="owl:ml-1 flex items-center leading-none text-right">
              <span class="w-24 px-2 py-1 text-right bg-white border border-gray-200 rounded">{{ realtimeData.doorBOut }}</span>
            </dd>
          </div>
...
const places = db && db.ref("/people_counting_opencv");
const places2 = db && db.ref("/people_counting_opencv2");
...
export default {
  ...
  data () {
    return {
      ...
      recipe: {
        ...
        items: [
          'Head Count', 'Population Density', 'Door A Enter Count', 'Door A Exit Count', 'Door B Enter Count', 'Door B Exit Count'
        ],
      },
    }
  },
  ...
  methods: {
    ...
    async init(){
    const doorA = (await places.once('value')).val();
    const doorB = (await places2.once('value')).val();

    this.realtimeData.doorAIn = doorA.totalLeft;
    this.realtimeData.doorAOut = doorA.totalRight;
    this.realtimeData.doorBIn = doorB.totalLeft;
    this.realtimeData.doorBOut = doorB.totalRight;
    console.log(doorA);
    ...
    places.on(
      ...
      x => {
        if (x.key == 'totalLeft'){
          this.realtimeData.doorAIn = x.val();
        }else {
          this.realtimeData.doorAOut = x.val();
        }
      }
    )
    places2.on(
      ...
      x => {
        if (x.key == 'totalLeft'){
          this.realtimeData.doorBIn = x.val();
        }else {
          this.realtimeData.doorBOut = x.val();
        }
      }
    )
    },
